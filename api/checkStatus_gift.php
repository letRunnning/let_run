<?php
    ...
    if ($con) {
        ...
        if($r_ID != '' ){
            ...
            $result_c = mysqli_query($con, $sql_c) or die("DB Error: Cannot retrieve message.");
            if($result_c->num_rows != 0){
                $sql_checkin = "SELECT * FROM `checkin` WHERE `registration_ID`='$r_ID'";
                $result_checkin = mysqli_query($con, $sql_checkin) or die("DB Error: Cannot retrieve message.");
                if($result_checkin->num_rows != 0){
                    $sql_red = "SELECT * FROM `redeem` WHERE `registration_ID`='$r_ID'";
                    $result_reedem = mysqli_query($con, $sql_red) or die("DB Error: Cannot retrieve message.");
