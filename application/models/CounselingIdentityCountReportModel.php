...
class CounselingIdentityCountReportModel extends CI_Model
{
  ...
  {
    $query = $this->db->query("SELECT counseling_identity_count_report.*, files.name as report_file_name FROM `counseling_identity_count_report` left join `files` on `counseling_identity_count_report`.report_file = `files`.no WHERE `project`=$county AND `month`=$month AND `year`=$year");
    if ($query->num_rows()) {
      return $query->row();
    }
  }
  ...
  function update_file_by_no($no, $reportFile) {
    ...
    return $this->db->update('counseling_identity_count_report', $this);
  }
