<?php
class NowYearsTrendSurveyCountReportModel extends CI_Model {
  /*
   * create course
   */
  function create_one($alreadyAttendingSchool, $alreadyWorking, $prepareToSchool, $prepareToWork, $noPlan, $lostContact,
    ...
    return ($this->db->insert('now_years_trend_survey_count_report', $this)) ? $this->db->insert_id() : '';
  }
  ...
  function create_one_new($one, $two, $three, $four, $five, $six,
    ...
    return ($this->db->insert('new_now_years_trend_survey_count_report', $this)) ? $this->db->insert_id() : '';
  }
  ...
  function update_one($alreadyAttendingSchool, $alreadyWorking, $prepareToSchool, $prepareToWork, $noPlan, $lostContact,
    ...
    return $this->db->update('now_years_trend_survey_count_report', $this);
  }
  ...
  function udpdate_one_new($one, $two, $three, $four, $five, $six,
    ...
    return $this->db->update('new_now_years_trend_survey_count_report', $this);
  }
  ...
  function get_by_no($year, $month, $county) {
    $this->db->select('new_now_years_trend_survey_count_report.*, files.name as report_file_name, (select Count(*) from youth where youth.source_school_year = ' . ($year-2) .' and youth.county = ' . $county . ') as youthCount');    
    $this->db->join('project', 'new_now_years_trend_survey_count_report.project = project.no');
    $this->db->join('files', 'new_now_years_trend_survey_count_report.report_file = files.no', 'left');
    $this->db->where('new_now_years_trend_survey_count_report.year', $year);
    $this->db->where('new_now_years_trend_survey_count_report.month', $month);
    $this->db->where('project.county', $county);

    $result = $this->db->get('new_now_years_trend_survey_count_report', 1)->row();
    return $result;
  }

  function get_by_all($year, $month, $county) {
    
    $this->db->select('county.name, new_now_years_trend_survey_count_report.*, (select Count(*) from youth where youth.source_school_year = ' . ($year-2) .' and youth.county = county.no) as youthCount');    
    $this->db->join('project', 'county.no = project.county');
    $this->db->join('new_now_years_trend_survey_count_report', 'new_now_years_trend_survey_count_report.project = project.no');
    $this->db->where('new_now_years_trend_survey_count_report.year', $year);
    $this->db->where('new_now_years_trend_survey_count_report.month', $month);
    if ($county != 'all') {
