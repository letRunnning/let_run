<?php
class HighSchoolTrendSurveyCountReportModel extends CI_Model
{
  /*
   * create course
   */
  ...
  return ($this->db->insert('new_high_school_trend_survey_count_report', $this)) ? $this->db->insert_id() : '';
}
  ...
  function udpdate_one_new($one, $two, $three, $four, $five, $six,
    ...
    return $this->db->update('new_high_school_trend_survey_count_report', $this);
  }
  ...
  {
    $this->db->select('new_high_school_trend_survey_count_report.*, files.name as report_file_name, (select Count(*) from youth where youth.source_school_year = ' . ($year-3) .' and youth.county = ' . $county . ') as youthCount');    
    $this->db->join('project', 'new_high_school_trend_survey_count_report.project = project.no');
    $this->db->join('files', 'new_high_school_trend_survey_count_report.report_file = files.no', 'left');
    $this->db->where('new_high_school_trend_survey_count_report.year', $year);
    $this->db->where('new_high_school_trend_survey_count_report.month', $month);
    $this->db->where('project.county', $county);

    $result = $this->db->get('new_high_school_trend_survey_count_report', 1)->row();
    return $result;
  }
  ...
  function get_by_all($year, $month, $county)
  {
    ...
    $this->db->select('county.name, new_high_school_trend_survey_count_report.* ,(select Count(*) from youth where youth.source_school_year = ' . ($year-3) .' and youth.county = county.no) as youthCount');
    $this->db->join('project', 'county.no = project.county');
    $this->db->join('new_high_school_trend_survey_count_report', 'new_high_school_trend_survey_count_report.project = project.no');
    $this->db->where('new_high_school_trend_survey_count_report.year', $year);
    $this->db->where('new_high_school_trend_survey_count_report.month', $month);
