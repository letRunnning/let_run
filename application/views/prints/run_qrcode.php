...
<div class="container" style="width:95%">
  <div class="row justify-content-center">
    <div class="col-4 text-right">
      <select onchange="location = this.value;" class="form-select mb-3" name="runActive" id="G-runActive" >
        <?php if($runID == null){?>
          <option selected value="<?php echo site_url('prints/run_qrcode/'); ?>">請選擇路跑活動</option>
          <?php foreach($activities as $i) {?>
          <option  <?php echo ($runID == $i['running_ID']) ? 'selected' : '' ?> value="<?php echo site_url('prints/run_qrcode/'.$i['running_ID']); ?>" ><?php echo $i['name']?></option>
          <?php } }else{ ?>
          <?php foreach($activities as $i) {?>
            <option  <?php echo ($runID == $i['running_ID']) ? 'selected' : '' ?> value="<?php echo site_url('prints/run_qrcode/'.$i['running_ID']); ?>" ><?php echo $i['name']?></option>
            <?php }} ?>
  ...
  <br>
    ...
    <table class="table text-center border-secondary table-hover align-middle">
        ...
        <tr>
            <th scope="col"><?php echo $activity->name?></th>
            
            <th scope="col"><img class="rounded" style="width:120px" src="<?php echo site_url() . '/files/qrcode/'. (string)base64_encode($runID).'.png'; ?>" /></th>
        </tr>
