				...
				<form action="<?php echo site_url($url); ?>" method="post" accept-charset="utf-8" enctype="multipart/form-data">
						...
						<div class="col-sm-10 col-md-8 mb-3">
							...
							<select class="form-select form-select-lg" name="years" id="years" onchange="location = this.value;">
								...
								<?php foreach ($years as $i) { ?>
									...
									<option <?php echo ($yearType == ($i['year'])) ? 'selected' : '' ?> value="<?php echo site_url('/report/counselor_manpower_report_organization_table/' . $i['year'] . '/' . $monthType); ?>"><?php echo $i['year'] ?></option>
								<?php } ?>
						...
						<div class="col-sm-10 col-md-8 mb-3">
							...
							<select class="form-select form-select-lg" name="months" id="months" onchange="location = this.value;">
								...
								<?php foreach ($months as $i) { ?>
									...
									<option <?php echo ($monthType == $i) ? 'selected' : '' ?> value="<?php echo site_url('/report/counselor_manpower_report_organization_table/' . $yearType . '/' . $i); ?>"><?php echo $i ?></option>
								<?php } ?>
					...
					<?php if ($reportProcessesCounselorStatus == $reviewStatus['review_process_pass']) : ?>
						...
						<table class="table table-hover table-bordered align-middle text-center" style="border:2px grey solid;">
							...
							<tbody>
								<tr>
									<td><?php echo $counselorManpowerReports ? $counselorManpowerReports->project_counselor : null ?></td>
									<td><?php echo $counselorManpowerReports ? $counselorManpowerReports->really_counselor : null ?></td>
									<td><?php echo $counselorManpowerReports ? $counselorManpowerReports->education_counselor : null ?></td>
									<td><?php echo $counselorManpowerReports ? $counselorManpowerReports->counseling_center_counselor : null ?></td>
									<td><?php echo $counselorManpowerReports ? $counselorManpowerReports->school_counselor : null ?></td>
									<td><?php echo $counselorManpowerReports ? $counselorManpowerReports->outsourcing_counselor : null ?></td>
									<td><?php echo $counselorManpowerReports ? $counselorManpowerReports->bachelor_degree : null ?></td>
									<td><?php echo $counselorManpowerReports ? $counselorManpowerReports->master_degree : null ?></td>
									<td><?php echo $counselorManpowerReports ? $counselorManpowerReports->qualification_three : null ?></td>
									<td><?php echo $counselorManpowerReports ? $counselorManpowerReports->qualification_four : null ?></td>
									<td><?php echo $counselorManpowerReports ? $counselorManpowerReports->qualification_one + $counselorManpowerReports->qualification_two
												+ $counselorManpowerReports->qualification_five + $counselorManpowerReports->qualification_six : null ?></td>
									<td style="text-align:left"><?php echo $counselorManpowerReports ? str_replace("\n", "<br/>", $counselorManpowerReports->note) : null ?></td>
								</tr>
							</tbody>
						</table>
					...
					<?php if ($reportProcessesCountyStatus == $reviewStatus['review_process_wait'] && $role == 3) : ?>
						...
						<?php if (!empty($counselorManpowerReports->report_file_name)) : ?>
							<?php if (strpos($counselorManpowerReports->report_file_name, 'pdf') !== false) : ?>
								<a class="col-sm-10 offset-md-2 col-md-8 link-primary" href="<?php echo site_url() . '/files/' . $counselorManpowerReports->report_file_name; ?>" download="<?php echo $yearType . '年' . $monthType . '月' . $title ?>"><?php echo $yearType . '年' . $monthType . '月' . $title ?></a>
							<?php else : ?>
...
<script type="text/javascript">
	...
	function exportTableToEXCEL(report, filename) {
		var csv = [];
