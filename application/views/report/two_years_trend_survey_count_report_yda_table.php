		...
		<div class="card-body col-sm-12">
			...
			<div class="card-content">
				...
				<form action="<?php echo site_url($url); ?>" method="post" accept-charset="utf-8" enctype="multipart/form-data">
						...
						<div class="col-sm-10 col-md-8 mb-3">
							...
							<select class="form-select form-select-lg" name="years" id="years" onchange="location = this.value;">
								...
								<?php foreach ($years as $i) { ?>
									...
									<option <?php echo ($yearType == ($i['year'])) ? 'selected' : '' ?> value="<?php echo site_url('/report/two_years_trend_survey_count_report_yda_table/' . $i['year'] . '/' . $monthType . '/' . $countyType); ?>"><?php echo $i['year'] ?></option>
								<?php } ?>
						...
						<div class="col-sm-10 col-md-8 mb-3">
							...
							<select class="form-select form-select-lg" name="months" id="months" onchange="location = this.value;">
								...
								<?php foreach ($months as $i) { ?>
									...
									<option <?php echo ($monthType == $i) ? 'selected' : '' ?> value="<?php echo site_url('/report/two_years_trend_survey_count_report_yda_table/' . $yearType . '/' . $i . '/' . $countyType); ?>"><?php echo $i ?></option>
								<?php } ?>
					...
					<div class="row justify-content-center">
						<div class="col-sm-10 col-md-8 mb-3">
							...
							<select class="form-select form-select-lg" name="counties" id="counties" onchange="location = this.value;">
								<option <?php echo ($countyType == 'all') ? 'selected' : '' ?> value="<?php echo site_url('/report/two_years_trend_survey_count_report_yda_table/' . $yearType . '/' . $monthType . '/all'); ?>">全部</option>
								<?php foreach ($countiesName as $i) { ?>
									<option <?php echo ($countyType == $i['no']) ? 'selected' : '' ?> value="<?php echo site_url('/report/two_years_trend_survey_count_report_yda_table/' . $yearType . '/' . $monthType . '/' . $i['no']); ?>"><?php echo $i['name'] ?></option>
								<?php } ?>
							</select>
						</div>
					</div>
					...
					<a class="btn btn-success" href="<?php echo site_url('export/yda_month_report_export/' . 'surveyTypeTwoYearsTrack' . '/' . $yearType . '/' . $monthType); ?>"><?php echo $title ?>表列印(下載EXCEL檔)</a>
					<br /><br />
									...
									<tr>
										...
										<?php } else { ?>
											...
											<td style="text-align:left"><?php $cnt = "";
																									...
																									?></td>
											<?php if ($countyType == 'all') : ?>
												<td><a class="btn btn-info" href="<?php echo site_url('report/two_years_trend_survey_count_report_yda_table/' . $yearType . '/' . $monthType . '/' . $counties[$j]['no']); ?>">審核</a></td>
							...
							<div class="row justify-content-center">
								<?php if (!empty($twoYearsTrendSurveyCountReportArray[$i]->report_file_name)) : ?>
									<?php if (strpos($twoYearsTrendSurveyCountReportArray[$i]->report_file_name, 'pdf') !== false) : ?>
										<a class="col-sm-10 col-md-8 link-primary" href="<?php echo site_url() . '/files/' . $twoYearsTrendSurveyCountReportArray[$i]->report_file_name; ?>" download="<?php echo $yearType . '年' . $monthType . '月' . $counties[0]['name'] ?>"><?php echo $yearType . '年' . $monthType . '月' . $counties[0]['name'] . '-' . $title ?></a>
									<?php else : ?>
										<div class="col-sm-10 col-md-8">
											<img class="figure-img img-fluid" src="<?php echo site_url() . '/files/' . $twoYearsTrendSurveyCountReportArray[$i]->report_file_name; ?>" />
										</div>
									<?php endif; ?>
								<?php endif; ?>
							</div>
